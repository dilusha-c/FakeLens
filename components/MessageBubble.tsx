...
export default function MessageBubble({ message }: MessageBubbleProps) {
  ...
  return (
    <div className={`flex items-start gap-2 sm:gap-3 ${isUser ? 'flex-row-reverse' : ''}`}>
      {/* Message Content */}
      <div className={`flex-1 max-w-[85%] sm:max-w-[80%] ${isUser ? 'items-end' : 'items-start'}`}>
        {message.imagePreview ? (
          ...
          <div className="w-full">
            <div className={`rounded-xl sm:rounded-2xl p-2.5 sm:p-3 mb-2 text-sm sm:text-base ${
              isUser
                ? 'bg-[var(--bg-tertiary)] text-[var(--text-primary)] ml-auto'
                : 'bg-[var(--bg-secondary)] text-[var(--text-primary)]'
            }`}>
          ...
          <div className={`rounded-xl sm:rounded-2xl p-3 sm:p-4 text-sm sm:text-base ${
            isUser
              ? 'bg-[var(--bg-tertiary)] text-[var(--text-primary)] ml-auto'
              : 'bg-[var(--bg-secondary)] text-[var(--text-primary)]'
          }`}>
